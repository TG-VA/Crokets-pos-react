-- Corrige el filtro de estado de get_commissions_report_data.
-- El enum canónico de ventas canceladas en el app es 'cancelled' (también
-- 'cancelada' en importaciones legacy); el filtro previo usaba 'canceled'
-- (una sola L) y nunca excluía esas ventas, pagando comisión por tickets
-- cancelados. Fuente del enum: ticketBuilder.js, reportsDashboardUtils.js,
-- salesHistoryService.js (status !== 'cancelled'), cashCut.jsx.
