-- Escalabilidad del reporte de caja (KNOWN_ISSUES #21).
-- 1) Indice compuesto para el join por ventana de sesion sobre sale_payments.
-- 2) Acota el CTE session_payments con los limites de las sesiones filtradas
--    (min(opened_at) y max(cierre o now())), evitando agregar TODO el historico.
-- El resultado es identico al anterior: la cota superior usa el maximo de las
-- ventanas, por lo que nunca descarta pagos que el JOIN final si contaria.

CREATE INDEX IF NOT EXISTS idx_sale_payments_branch_created_at
  ON public.sale_payments (branch_id, created_at);
...
AS $$
  ...
  session_payments AS (
    ...
    WHERE sl.status IN ('completed', 'partial_refund')
      AND sp.branch_id IN (SELECT branch_id FROM filtered_sessions)
      AND sp.created_at >= (SELECT MIN(opened_at) FROM filtered_sessions)
      AND sp.created_at <= (SELECT MAX(COALESCE(closed_at, now())) FROM filtered_sessions)
  )
  SELECT
    fs.id AS session_id,
    fs.user_id,
