-- Ajusta get_commissions_report_data para devolver TODAS las filas del periodo
-- cuando no se envían parámetros de paginación (p_page_size NULL = sin límite).
-- El reporte agrega KPIs, resúmenes por cajero/producto y exporta con la vista
-- completa; la paginación visual es en cliente (usePagination en las tablas).
