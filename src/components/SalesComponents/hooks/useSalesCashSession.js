...
const useSalesCashSession = ({
  ...
  const updateLocalShiftCutFlag =
    useCallback((alreadyCut) => {
      if (alreadyCut) {
        localStorage.setItem(
          SHIFT_CUT_STORAGE_KEY,
          "true"
        );
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(
        `sales-cash-session-${branchId}-${userId}`
      )
      .on(
        "postgres_changes",
        {
