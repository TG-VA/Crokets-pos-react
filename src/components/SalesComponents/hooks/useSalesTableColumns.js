...
const DEFAULT_COLUMN_WIDTHS = [400, 150, 80, 150, 150];
const DEFAULT_COLUMN_PROPORTIONS = [0.4, 0.15, 0.1, 0.15];
...
const useSalesTableColumns = ({
  ...
  // Inicialización de columnas basada en porcentajes y el tamaño real del contenedor
  useEffect(() => {
    if (!tableRef.current || isInitialized) return;

    const tableWidth = tableRef.current.offsetWidth;
    const availableWidth = tableWidth - tableHorizontalOffset;

    const calculatedWidths = columnProportions.map((proportion) =>
      Math.max(minColumnWidth, Math.floor(availableWidth * proportion))
    );
  ...
  }, [columnProportions, isInitialized, minColumnWidth, tableHorizontalOffset]);
