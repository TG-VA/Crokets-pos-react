...
const buildRewardRowsFromCartItems = ({ rewardItems = [], customerId = null }) => {
  return (rewardItems || [])
    .filter((item) => (item?.is_reward_item || item?.is_reward_discount_item) && item?.reward_id && Number(item?.cantidad || 0) > 0)
    .map((item) => {
...
const loadRewardRedemptionsForPrintedSale = async ({ saleId, fallbackRewardItems = [], customerId = null }) => {
  const fallbackRows = buildRewardRowsFromCartItems({ rewardItems: fallbackRewardItems, customerId });
  if (!saleId) return fallbackRows;

  try {
    const { data, error } = await supabase
      .from("sale_reward_redemptions")
      .select("id, sale_id, sale_detail_id, customer_id, reward_id, product_id, quantity, points_per_unit, total_points, unit_price, discount_amount, reward_name, product_name, created_at")
      .eq("sale_id", saleId)
      .order("created_at", { ascending: true });

    if (error) throw error;
    return (data || []).length > 0 ? data : fallbackRows;
  } catch (error) {
...
export const printSaleTicket = async ({
  saleId, paymentData, paymentPayload, notes, saleDate, saleClient = null, pointsResult = null, cartItems = [], branch = null, user = null, subtotal = 0, discountTotal = 0, total = 0,
}) => {
  try {
    ...
    const [detailsRes, kitItemsRes, rewardRedemptions] = await Promise.all([
      supabase
        .from("sale_details")
        .select("id, quantity, unit_price, total_price, product_id, original_unit_price, final_unit_price, discount_type, discount_value, discount_amount")
        .eq("sale_id", saleId),
      supabase
        .from("sale_kit_items")
        .select("id, sale_id, sale_detail_id, kit_product_id, component_product_id, quantity")
        .eq("sale_id", saleId),
      loadRewardRedemptionsForPrintedSale({
        saleId,
        fallbackRewardItems: pointsResult?.rewardRedemptions?.length ? [] : rewardItemsForPrint,
        customerId: saleClient?.id || null,
      }),
    ]);
    ...
    const rewardRowsForPrint = pointsResult?.rewardRedemptions?.length > 0 ? pointsResult.rewardRedemptions : rewardRedemptions;

    const rewardBySaleDetailId = {};
    const rewardByProductId = {};
    for (const reward of rewardRowsForPrint || []) {
      if (reward?.sale_detail_id) rewardBySaleDetailId[reward.sale_detail_id] = reward;
      if (reward?.product_id && !rewardByProductId[reward.product_id]) rewardByProductId[reward.product_id] = reward;
    }

    const productIds = [...new Set([
      ...detailsRows.map((d) => d.product_id),
      ...kitItemRows.map((k) => k.component_product_id),
      ...(rewardRowsForPrint || []).map((r) => r.product_id),
    ].filter(Boolean))];

    const { data: productsRows, error: productsError } = productIds.length
      ? await supabase.from("products").select("id, name, barcode, is_kit").in("id", productIds)
      : { data: [], error: null };
    ...
    for (const product of productsRows || []) {
      ...
      productIsKitMap[product.id] = Boolean(product.is_kit);
    }

    const kitItemsByDetail = {};
    for (const row of kitItemRows) {
      if (!kitItemsByDetail[row.sale_detail_id]) kitItemsByDetail[row.sale_detail_id] = [];
    ...
    let itemsForPrint = detailsRows.map((item) => {
      ...
      const rewardInfo = rewardBySaleDetailId[item.id] || (Number(item.total_price || 0) === 0 ? rewardByProductId[item.product_id] : null);
      const isRewardLine = Boolean(rewardInfo) && Number(item.total_price || 0) === 0;
      const originalUnitPrice = Number(item.original_unit_price || rewardInfo?.unit_price || item.unit_price || 0);

      return {
        quantity: Number(item.quantity || 0),
        description: productMap[item.product_id] || rewardInfo?.product_name || "PRODUCTO",
        unit_price: isRewardLine ? 0 : Number(item.final_unit_price || item.unit_price || 0),
        original_unit_price: originalUnitPrice,
        discount_amount: isRewardLine ? 0 : Number(item.discount_amount || 0),
        line_total: Number(item.total_price || 0),
        is_kit: Boolean(productIsKitMap[item.product_id]) || components.length > 0,
        components: components.map((c) => ({ quantity: c.quantity, description: c.description })),
    ...
    if (itemsForPrint.length === 0 && (rewardRowsForPrint || []).length > 0) {
      itemsForPrint = (rewardRowsForPrint || []).map((reward) => ({
        ...
        description: productMap[reward.product_id] || reward.product_name || "PRODUCTO",
        unit_price: 0, original_unit_price: Number(reward.unit_price || 0),
        discount_amount: 0, line_total: 0, is_kit: false, components: [],
        is_reward_item: true, isRewardItem: true,
        reward_id: reward.reward_id || null, rewardId: reward.reward_id || null,
        reward_name: reward.reward_name || "RECOMPENSA", rewardName: reward.reward_name || "RECOMPENSA",
        reward_points: Number(reward.points_per_unit || 0), rewardPoints: Number(reward.points_per_unit || 0),
        total_points: Number(reward.total_points || 0), totalPoints: Number(reward.total_points || 0),
        sale_reward_redemption_id: reward.id || null, saleRewardRedemptionId: reward.id || null,
      }));
    }

    const rewardPointsUsed = Number(pointsResult?.pointsUsed || 0) || (rewardRowsForPrint || []).reduce((sum, r) => sum + Number(r.total_points || 0), 0);
    const rewardsCount = (rewardRowsForPrint || []).reduce((sum, r) => sum + Number(r.quantity || 0), 0);
    const hasRewardRedemptions = rewardRowsForPrint.length > 0 || rewardPointsUsed > 0 || rewardsCount > 0;
    const isRewardOnlySale = hasRewardRedemptions && Number(total || 0) <= 0;

    const paymentsForTicket = isRewardOnlySale ? [] : (paymentPayload || []).filter((p) => Number(p.amount || 0) > 0);
    const totalPaid = paymentsForTicket.reduce((sum, p) => {
      const amount = Number(p.amount || 0);
      const currency = String(p.currency || "MXN").toUpperCase();
      const exchangeRate = Number(p.exchange_rate || 0);
      return sum + (currency === "USD" ? (exchangeRate > 0 ? amount * exchangeRate : 0) : amount);
    }, 0);

    const cashierName = (user?.username || user?.email || "CAJERO").toUpperCase();
    ...
    const ticketText = buildTicketText({
      ...
      sale: {
        ...
        amount_received: isRewardOnlySale ? 0 : totalPaid || Number(total || 0),
        change_amount: isRewardOnlySale ? 0 : Math.max(Number(paymentData?.change || 0), 0),
        ...
        customer_points_balance: pointsResult?.newBalance !== undefined && pointsResult?.newBalance !== null ? Number(pointsResult.newBalance) : null,
      },
      items: itemsForPrint,
      cashierName,
      footer: {
