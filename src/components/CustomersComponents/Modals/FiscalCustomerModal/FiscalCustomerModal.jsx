...
const FiscalCustomerModal = ({
  ...
  const phoneDigits = onlyNumbers(form.phone);
  const phoneConfirmDigits = onlyNumbers(form.phoneConfirm);
  ...
  const fieldStatus = {
    ...
    phoneConfirm:
      form.phoneConfirm.length === 0
        ? ""
        : phoneConfirmDigits.length === 10 && phoneConfirmDigits === phoneDigits
        ? "valid"
        : "invalid",
  ...
  useEffect(() => {
    ...
    if (customerToEdit?.id) {
      const currentPhone = onlyNumbers(customerToEdit.phone || "").slice(0, 10);
      const postalCode = onlyNumbers(customerToEdit.postal_code || "").slice(
        0,
        5
      );
      ...
      setForm({
        ...
        fiscal_email: normalizeEmail(
          customerToEdit.fiscal_email || customerToEdit.email || ""
        ),
        ...
        is_points_customer: customerToEdit.is_points_customer === true,
      });
  ...
  const handleSelectCustomer = (customer) => {
    const currentPhone = onlyNumbers(customer.phone || "").slice(0, 10);
    ...
    setForm({
      ...
      is_points_customer: customer.is_points_customer === true,
    });
  ...
  const validateForm = () => {
    ...
    if (phoneConfirmDigits.length !== 10) {
      return "La confirmación del teléfono debe tener 10 dígitos.";
    }

    if (phoneDigits !== phoneConfirmDigits) {
      return "El teléfono y la confirmación no coinciden.";
    }
  ...
  const confirmPhoneChangeIfNeeded = (newPhone) => {
    const originalPhone = onlyNumbers(form.originalPhone || "");
    const phoneWasChanged =
      !!form.customerId && originalPhone && originalPhone !== newPhone;
    ...
    const pointsWarning =
      form.is_points_customer === true
        ? "\n\nEste cliente también está registrado como cliente de puntos. Al cambiar el teléfono, también se actualizará el teléfono asociado a sus puntos."
        : "";

    return window.confirm(
      `Cambiaste el teléfono fiscal del cliente.\n\nTeléfono anterior: ${
        originalPhone || "SIN TELÉFONO"
      }\nTeléfono nuevo: ${
        newPhone || "SIN TELÉFONO"
      }\n\nEl teléfono se usa para vincular clientes fiscales con clientes de puntos.${pointsWarning}\n\n¿Deseas continuar?`
    );
  };
  ...
  const handleSave = async () => {
    ...
    const phoneChangeConfirmed = confirmPhoneChangeIfNeeded(phone);

    if (!phoneChangeConfirmed) {
      return;
    }
          ...
          <div className={styles.formSection}>
            <div className={styles.formGrid}>
              <div className={styles.field}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField(
                      "phone",
                      onlyNumbers(e.target.value).slice(0, 10)
                    )
                  }
                  maxLength={10}
                />
              ...
              <div className={styles.field}>
                ...
                <input
                  className={getInputClass("phoneConfirm")}
                  ...
                  maxLength={10}
                />
                {fieldStatus.phoneConfirm === "invalid" && (
                  <small className={styles.fieldError}>
                    {phoneConfirmDigits.length !== 10
                      ? "La confirmación debe tener 10 dígitos."
              ...
              <div className={styles.field}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField(
                      "rfc",
                      normalizeRFC(e.target.value).slice(0, 13)
                    )
                  }
                  maxLength={13}
                />
